...
-- (the owner's QA feedback). "fill" (cover, default) keeps the art's aspect and
...
-- (a radial sweep stretched over the full 2:1 plate reads as an ellipse — the owner's call,
...
-- Dim-on-cooldown (session 12, the owner's ask): the plate colour darkens while the action's
...
-- tint — which reads clean/legible (the owner's call, 2026-07-19).
local MOD_ICON = {
...
-- looks CONSISTENT across differently-sized bars (the owner: same preset on pet +
...
local function ApplyHotkeyOverride(btn)
  ...
  -- SetTextColor, so pet keybinds flickered orange→grey every frame (the owner). Pet
  ...
  -- as the owner's stray "dot". It's a placeholder, never a real bind → keep it hidden.
...
local function ApplyDecor(btn)
  ...
  -- wrapping both (the owner's mock). Continuous OFF masks the icon to its OWN shape
  ...
  -- midline edge hides under the opaque end of the fade (the owner, session 12). Still below
...
function Skin:ReapplyDecor()
  ...
  GB:ForEachButton(function(btn)
    ...
    -- never corrected (the owner: tall Pill edit-preset stretched his 1:1 Hexagon/Square
...
local function AlignCooldowns(btn)
  ...
  -- fixed Blizzard's undershoot — so it's baked, not a user slider (the owner 2026-07-20).
  ...
  -- stays circular there; the full-plate anchor made it elliptical — the owner, session 12;
...
local function computeIconTint(btn)
  ...
  if pv("rangeTint") and rec.outOfRange then
    ...
    -- that lets the icon's own colours bleed through). The owner 2026-07-20.
...
local function handIconSize(btn)
  ...
  -- so a preset's size scale would overlap neighbours (the owner, session 13).
...
-- stance have no UpdateFlash and it fires all through combat (the owner: the square came
...
-- 4 comets, ~5s orbit) — the owner wants it always-on for autocast, not configurable.
...
-- the owner) is declared at the top of the file so RestoreButton can stop it.
...
-- re-dressed (the owner: several bars on screen look alike). Renders the shape's
...
-- ---------------------------------------------------------------------------
-- Preset-focus highlight (session 15, the owner): a persistent 50%-opacity purple
...
-- ★ Z-ORDER (the owner's requirement): the block must sit BELOW all action bars +
...
local HL_ALPHA = 0.5                -- the owner: 50% opacity
