...
-- ---------------------------------------------------------------------------
-- Shape registry — the skin engine is SHAPE-AGNOSTIC (the owner's requirement:
...
GB.STYLES = {
  none = {},
  -- the owner's mockup (2026-07-18): the button is TALLER than the icon — extra
...
-- ---------------------------------------------------------------------------
-- Profiles → presets → per-bar assignment (session 13; the owner's architecture:
...
-- character active, shared account-wide library — the owner, session 13).
--
...
-- with the owner). Everything else in the db (schema, skinEnabled, migration
...
-- Profile management (the owner: create, copy, rename, delete + per-character active).
...
-- it during combat taints the action buttons (the owner's delve bug: switching
