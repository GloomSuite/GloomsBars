...
## Settled decisions (2026-07-18 — do not reopen without the owner)
...
- **In-game UI follows the GloomsAuras design language**: same tokens (bright purple
  ...
  textures/widgets, pixel-perfect to mocks. The owner has **Figma mockups for GloomsAuras**
  to reference as the styling basis (Figma desktop MCP tools may be available; otherwise ask him for screenshots).
- Reuse GloomsAuras's config toolkit patterns (`~/GloomsAuras/Config.lua`) when the options UI phase starts.
...
QA is done by the owner (non-developer): give ONE copy-paste instruction at a time, verify
