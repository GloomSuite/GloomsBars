...
# Per-corner rounding (the owner's ask, 2026-07-18): each corner is independently
...
# Compared in-game via /gb glowstyle on the owner's tall pill; the winner graduates
...
def gen_test():
    # Bake the candidate glows on the owner's exact tall pill (circle @ 36x57) so he
...
def ring_alpha(d):
    ...
    # (the owner 2026-07-20): the rim peaks at FULL alpha over a wider band with a
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "glows":
        ...
        gen_test()    # candidate proc-glow profiles on the owner's tall pill (/gb glowstyle)
