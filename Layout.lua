...
-- count } — per bar, per PROFILE (settled with the owner: layout is bar geometry,
...
-- OUTSIDE the grid/slot events above (the owner: a timewalking dungeon un-hid three
...
-- The MASTER switch (the owner: layout is all-or-nothing — one switch says
...
local function applyBar(barKey)
  ...
  -- poisoned isShownExternal, which then blocked our own show path (the owner's
...
local function releaseBar(barKey)
  ...
  -- the owner's "Rows don't revert" bug).
...
-- (the owner's spec). ESC or the Config button exits; entering combat exits
...
-- Live coordinates on the overlay, CENTER-RELATIVE (the owner: 0 on X = perfectly
