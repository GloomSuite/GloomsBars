...
| Assisted-combat **highlight** | `AssistedCombatManager:SetAssistedHighlightFrameShown` → `AssistedCombatHighlightFrame.Flipbook` (`rotationhelper_ants_flipbook`) | **yes (glow)** | ✅ mechanism | shaped assist glow (blue); low priority per the owner |
...
## Glow architecture — multi-part & tintable (DECIDED 2026-07-20, the owner's mockups)
...
neighbours is acceptable (the owner: fine, maybe preferred) — governed by frame level, tunable later.
