...
| 3 | **`MaskTexture` renders in Midnight** (rounded corners + shaped sweeps depend on it) | 🟡 PARTIAL — v1 probe gave "corners slightly more rounded", NOT full circles. Hypothesis: buttons already carry Blizzard's `.IconMask` (default rounding) and v1's `TempPortraitAlphaMask` file may not have loaded. Probe v2 (swap Blizzard's IconMask atlas → `CircleMaskScalable`) written, awaiting QA. | `/gb mask` v2 + `/gb debug` (now reports `.IconMask` + its atlas) |
