...
and `Media/ui/{cmd,shift,ctrl,opt}.png`. Offer the owner a commit. Git history holds the older narrative;
...
Session 6 ended clean (the owner: "I think we're good"). Everything built this session was QA'd in-game and
...
3. **Aspect proc-glow art** (`pill-*-glow`) if the owner wants tall-shape glows even (follow-up a above).
