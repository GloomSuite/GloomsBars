...
- UI: **sliding switches** over checkboxes; **no native Blizzard UI** textures/widgets;
  **pixel-perfect** to any mock. Styling follows **GloomsAuras' design language** —
  The owner has Figma mockups for GloomsAuras as the reference basis.
...
- Siblings for reference (read-only): GloomsAuras at `~/GloomsAuras`
...
## Settled decisions (2026-07-18, with the owner — do not reopen)
...
| 6 | Proc glows are hookable without secret reads | ✅ VERIFIED 2026-07-18 — implemented via `ActionButtonSpellAlertManager` hooks (not the legacy events); **real in-combat proc produced our shaped gold halo on a round icon. THE DIFFERENTIATOR IS PROVEN.** the owner: needs visibility/intensity controls (Config backlog). | in-combat QA |
...
- **Session 1 (2026-07-18):** repo scaffolded (TOC, Core.lua w/ tokens + saved vars +
  ...
  - GitHub repo live: https://github.com/HandofDevastation/GloomsBars (public,
    ...
    installed + authorized on the owner's machine (the org admin account,
...
4. The owner's noted remaining square offenders: cooldown swipe (addressed next, see 5),
...
6. ❌ Masking the state textures FAILED in QA (hover stayed square — API-NOTES §2)
   ...
   **📌 PINNED (the owner): state glows are dimmer than default — color/opacity/intensity
...
7. ⚠ REVISED (2026-07-18): the persistent blue square is **NOT** the rotation-helper
   frame — the owner disabled the rotation assistant and the glow REMAINED. Source says
   ...
   rotation-helper frame if the owner re-enables that feature; QA deferred).
8. **FLEXIBILITY ARCHITECTURE (the owner's explicit requirement 2026-07-18 — do not
...
9. ✅ Built: **THE GLOW PHASE — `Glows.lua`**. One shaped additive halo per button
   ...
   **The blue-glow saga's final answer:** it was `AssistedCombatHighlightFrame` —
   ...
   combat — hence "no pulse"; the owner's decisive test: the game's highlight-assist
   setting kills it). ⚠ the owner's prioritization (2026-07-18): the assist highlight
   is LOW PRIORITY — the hook exists but do NOT iterate/QA it further; if it
   works, bonus. **→ Bonus landed (2026-07-18): after /reload the owner observed the
...
10. ✅ **QA'd 2026-07-18 — THE CORE DIFFERENTIATOR CASE PASSED**: real in-combat
    proc → shaped pulsing gold halo following the round icon (screenshot). The owner:
...
13. ✅ **DECORATION ENGINE — QA'd against the mock (2026-07-18, the owner: "pretty
...
14. ▶▶ **NEXT MAJOR BUILD: THE STYLE EDITOR (Config UI)** — the product, per the
    ...
    - Editor controls: extension size, gradient layers (color/position/fade
      ...
      glow styling (tint/intensity/pulse — the owner: current glows too subtle),
      state-glow colors/opacity, sweep overshoot.
    - Built in the GloomsAuras design language (toolkit port from
      `~/GloomsAuras/Config.lua`): sliding switches, no Blizzard
      widgets, pixel-perfect to the owner's Figma mocks. The owner's Figma numbers
...
## ★★ NORTH STAR CORRECTED (the owner, 2026-07-18 — supersedes nuance below):
**Styles must be USER-AUTHORED through the UI, not shipped presets.** the owner: "I
...
## ★ DESIGN NORTH STAR (the owner, 2026-07-18) — heavily customized button looks
The owner's mockup: a rounded-rect icon with an **orange gradient plate covering the
...
The owner's explicit expectation (2026-07-18): slash commands are DEV SCAFFOLDING only;
...
pixel-perfect; the owner's GloomsAuras Figma mockups = styling reference).
...
- 📌 State-glow styling: color / opacity / intensity per state (hover, checked,
  flash, assist) — the owner: current hover is too dim vs default
- Text styling (fonts/sizes; hotkey color = needs hook, see API-NOTES §3) —
  incl. hotkey font picker (the owner finds GeneralSans bland; try Khand as default)
...
## NEXT (continued)
11. Sometime: test WoWUp install-from-URL **on another machine** (NOT the owner's dev
...
3. Sometime: test WoWUp install-from-URL **on another machine** (NOT the owner's dev
...
- **2026-07-18: HotKey styler mystery SOLVED — it was Dominos' hotkey styler module**
  (the owner had forgotten it was installed; the Lato font FILE it applied ships with
  Platynator, a nameplate addon — LSM cross-pollination). **the owner removed it**; our
  GeneralSans hotkey font now applies (all three text swaps verified working).
  The owner's verdict: "General Sans is a pretty bland looking font" → hotkey font
...
- **2026-07-18: The `EQOL_ActionBarName` foreign member = EnhanceQoL** (UI-tweak suite).
  ...
  EQOL. The owner disabled it (2026-07-18) for a clean baseline. Long-term, Gloom's Bars owns
