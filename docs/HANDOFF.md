...
## ▶ FIRST THING NEXT SESSION (the owner): fix the STRETCHED ROUNDED CORNERS.
...
(see NEXT #1): the full 16-pattern × aspect × radius matrix is large. Decide the approach with the owner
...
1. **★ FIRST: fix STRETCHED ROUNDED CORNERS on mixed-corner non-square icons.** Aspect masks only cover
   ...
   matrix is large → SCOPE DECISION with the owner FIRST (restrict mixed corners to square icons? generate a
...
2. **Wire the cast-fill Config controls the owner asked for**: fill **direction** (up/down/left/right),
...
- Anytime: densify `PILL_RATIOS` if nearest-aspect snapping stretches caps at odd sizes; assist-frame
  border still base art (low priority, the owner: don't iterate); proc-glow aspect art (soft halo currently
