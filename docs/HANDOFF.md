...
enlarge, so it stays square on a pill. **DECISION (the owner, 2026-07-19): replace Blizzard's cast/channel
...
2. **Mixed-corner shapes on non-square icons still ovalize** — aspect masks only cover the all-rounded
   ...
   large → SCOPE DECISION with the owner (restrict mixed-corners to square icons? 9-slice fallback for
...
- Anytime: densify `PILL_RATIOS` if nearest-aspect snapping stretches caps at odd sizes; assist-frame
  border still base art (low priority, the owner: don't iterate).
