...
| 3 | **`MaskTexture` renders in Midnight** (rounded corners + shaped sweeps depend on it) | 🟡 IN PROGRESS — v1 "slightly more rounded" (ambiguous). v2: swapping Blizzard's `.IconMask` atlas → `CircleMaskScalable` succeeded 12/12, atlas exists, zero errors, **NO visual change**. Confound: the owner runs **ArcUI** (restyles action buttons, possibly draws its own icon). v3 = standalone texture+mask, screen center, addon-free — isolates whether masks render at all. `/gb maskinfo` inspects icon↔mask attachment. | `/gb mask` v3 + `/gb maskinfo` |
...
- **Session 1 (2026-07-18):** repo scaffolded (TOC, Core.lua w/ tokens + saved vars +
  ...
  - GitHub repo live: https://github.com/HandofDevastation/GloomsBars (public,
    HandofDevastation org). `gh` CLI installed + authorized on the owner's machine
    (the org admin account, scopes repo/workflow/read:org).
...
2. Confirm the `v0.0.1` release pipeline ran green (Actions tab / `gh run list`),
   then have the owner test WoWUp install-from-URL with the repo URL.
