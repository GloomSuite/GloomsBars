# Gloom's Bars — ARCHIVE

> **History. Not current state.** Every session record, newest first.
>
...
> HANDOFF wins.
>
> **You almost never need to read this** — come here only for the full reasoning behind a settled
> decision. Suite-wide history lives in `~/GloomsHub/docs/ARCHIVE.md`.
...
Config UX improvements + a new PRESET-FOCUS HIGHLIGHT feature, and fixed TWO nasty in-play bugs the owner hit
live (a combat taint that froze cooldowns, and pet keybind colour reverting to grey). ★★ THE BIG PROCESS
LESSON (the owner watched me swirl HARD on the keybind bug): when a rendered value (colour/font/anchor) is wrong
...
a Claude-authored addon the owner had) in ONE reload, and a SetVertexColor trap cracked the pet colour. See
...
in UI text/chat; GUI-only for user controls (dev slash probes OK, the owner wants them KEPT until the build
...
### PART B — CONFIG UX WAVE (`adf8dc4`). All QA'd by the owner.
...
- **Text chip control REORDER (the "bonkers" item):** OUTLINE & SHADOW now follows POSITION in all four Text
  ...
  ★ the owner noted spacing is still a touch inconsistent but it's LOW priority — leave it.
...
### PART C — PRESET-FOCUS HIGHLIGHT (`ad881b4`, the owner's feature). Built + QA'd.
...
which bars your edits affect + you can watch live previews on them. ★ the owner's constraints, all honoured:
...
- **Every flatButton is now PURPLE off/unselected, ORANGE on/selected** (baked into `flatButton`'s SetActive —
  ...
  layer cells, footer toggles — one change, universal. The owner's rule.
...
### PART E — RefreshAll COMBAT-GATE (`2c2ee75`). ★ A REAL TAINT BUG the owner hit live in a delve.
...
- **★★ THE PROCESS FAILURE (the owner called it out, rightly):** I spent ~8 rounds + many reloads INFERRING and
  ...
  - The ACTION-BAR keybinds showing green Lato (a DIFFERENT bug, surfaced same session) = **DominosHotkeyStyler**
    (a Claude-authored addon the owner had for Dominos, no longer used) re-styling on a 1s/3s/5s login timer +
    UpdateHotkeys hook. The owner DELETED it → fixed. Not GB.
...
- **Deep-in-the-weeds debugging protocol (the owner):** he'll keep testing/reloading to help isolate, but expects
...
- DominosHotkeyStyler (the owner's leftover Claude-authored addon) deleted by the owner — it was the action-bar
...
(mode row, not a toggle — the owner caught that off just meant "Blizzard style", not "gone"). ApplyNameOverride
...
portrait silhouettes OVERLAPPED (the owner screenshot). Members clamp to natural size (preset sizeScale also
...
### PART C — PROFILES (the owner's architecture, settled): **profiles → whole-look presets → per-bar assignment.**
...
- **Stage 2 (`a533bd9`):** "Profiles" section FIRST in the accordion: profile picker + New/Copy/Rename/
  ...
  (attachTip — dark plate + purple title, REUSABLE) replaced the hint block** (the owner: "giant ugly").
...
- **Per-character default (`f3f138f` era):** a character's FIRST login auto-creates its OWN profile,
  **"Name - Realm" (the GloomsAuras convention — the owner cited it)**; existing bindings honored. Footer
  "Profile: X" button (an unwired increment-1 stub the owner found) = the live profile SWITCHER (`3455d03`).

### PART D — BAR LAYOUT (Layout.lua, phases L1–L4). The owner's scope: size, gaps, rows/cols, orientation,
...
- **★ THE FOUNDATION:** Blizzard lays out each bar's **BUTTON CONTAINERS — plain UNPROTECTED Frames**
  ...
  explained to the owner: Button size = geometry per bar; Shape&icon Size = icon-in-button, per preset; they
...
- **MASTER SWITCH** (the owner: all-or-nothing): `profile.layoutEnabled` arranges ALL bars; per-bar tables are
...
- **Gaps:** main-axis Gap + cross-axis **Row gap** (only visible at Rows>1, ABOVE Orientation), both
  **−32..64** (hex/circle silhouettes don't fill their square rects — negative overlap nestles them;
  step clamped ≥4px; caveat told to the owner: hit-rects overlap slightly). **Copy layout from** dropdown
...
- **Position (L3, `8812299`):** movers (translucent purple, UIParent-parented so hidden bars are movable),
  ...
  The owner wants exact-0 centring). Button reads "Move bars"/"Lock bars" (names the NEXT action).
...
### ★★ THE OWNER-DIRECTION (saved to memory, but re-read these):
...
REVERTED — the owner didn't want it), `adcbddb` (13 preview chips), `6de407c`/`91e99c9`/`f3c632b`/`e63c9f2` (caption
evolution), `dc1db56` (spacing + cast-drain preview + countdown controls), `4173b77`/`bfc2174`/`802d967`
(cast-complete preview burst added → fixed → REMOVED per the owner), `8104465` (Text consolidation), `2b36c72`
...
2. **Green equipped-item border reappearing.** Root cause FOUND in client source (`ActionBarActionButtonMixin:
   ...
   re-asserting alpha 0 while the skin is enabled (in `Suppress`). ⚠ No deterministic repro — the owner watching.
3. **Plate cooldown ellipse (the owner's design call: icon-square-only).** `AlignCooldowns` hand path now anchors
   ...
   hard midline edge hides under the opaque end of the fade (the owner's "no weird bottom edge" ask). Still below
...
- **Consolidation (`8104465`, the owner: "scattered and nonintuitive"):** the **Text section** now has a CHIP ROW
...
### PART H — Preview-pane upgrade (the owner drove the design; several iterations):
...
- **Captions (`6de407c`→`e63c9f2`):** each state = { bold Semibold HEADING, the owner's descriptive prose, a bold
  **"Styled in:" bullet list** of clickable section links }. Links = `|Hgbsec:<title>|h` in caret orange on a
  ...
  channel — fill & bursts", "Text — countdown numbers"). ALL left-aligned (the owner: you can't centre a bullet
...
- **Cast-complete burst: REMOVED (`802d967`).** Tried twice (`4173b77` shared burst player; `bfc2174` fixed it
  ...
  lookalike from our flash player reads wrong. The owner's rule: **better no preview than an unfaithful one.**
  (The cooldown finish-flash preview stays — it mirrors OUR OWN bar animation 1:1.)
- **Preview keybind chip: REVERTED (`f7d7d5b`).** the owner: bars show the real thing; not worth preview drift.
...
- **Preview honesty is a the owner design principle:** preview elements must mirror the ENGINE's exact art +
...
- **the owner interrupted a design I was about to build and supplied his own** (caption links → his bullet-list
  spec). When he starts "Wait—", STOP and let him finish before writing code.

### ✔ CLOSED / DROPPED this session:
- **Coexistence QA: CLOSED** — the owner's full addon ecosystem (ArcUI, EnhanceQoL, etc.) was enabled THE WHOLE
  TIME with zero issues; the handoff's standing "re-test with addons enabled" item is done.
- **Per-trigger glow spread/softness: DROPPED by the owner** — obsolete now that the per-trigger matrix exists.
- **Out-of-range is the ONLY state that touches keybind text** (Blizzard's range indicator colours it red;
  we recolour to match). OOM/unusable tint the icon only. (Answered for the owner.)
...
3. **Cooldown radial sweep looks elliptical/diagonal on the tall plate.** Blizzard's cooldown is a RADIAL wedge;
   ...
   (cooldown REMAINING time is secret — unlike cast time). **DESIGN DECISION for the owner:** (a) accept the radial
...
A long, high-throughput session done GUI-first (the owner HATES slash commands — see below). Two big deliverables.
...
- **Per-trigger EVERYTHING, never global.** Glow (colour/opacity/layers) AND animation params are per-STATE.
  (the owner: an animation's settings must NOT be global — Proc red-CW and Hover green-CCW coexist.)
...
- **GUI ONLY — NO slash commands for the user.** the owner "really really really" HATES slash/CLI (saved to memory
...
3. **Glow leftover (#1 remainder):** per-trigger **spread/softness** for the glow (the owner said OPTIONAL — the
...
in-game as a dev preview. Structured into phases at the owner's request (he wanted a frozen plan, not reactive
feature-chasing — see memory `work-structured-catalog-first`).

**THE PIVOT (settled with the owner, do not reopen):** free width/height sizing produced warped glows/overlays
...
**★ Multi-part glow architecture (the owner's design, validated).** Every glow = the SAME 3 parts, differing only
...
**★ Hand-authored assets (the owner makes them in Figma; ALL 21 done, imported to `Media/art/hand/`).** Spec +
...
The owner's originals live at `~/Desktop/gb_assets`; imported+keyed copies in `Media/art/hand/` (63 files).
...
- **Mask textures need WHITE rgb, not just alpha.** Figma exports transparent regions as BLACK rgb (0,0,0,0);
  ...
  **won't clip**. Fix applied: a script forced RGB→255 (alpha untouched) on all `Media/art/hand/*-base.png`.
  If the owner re-exports a base, re-whiten its RGB.
...
fork — rows/gap; Edit-Mode-owned; scope decision still OPEN with the owner) · Apply to bars
...
`PILL_RATIOS` if the owner notices.
...
TWO-PIECE border (offered to the owner, deferred). (c) The PREVIEW still doesn't render the plate/extension
...
gaps. Everything below was verified in-game (the owner: "I think we're good"). Base `829a96f`.
...
**Mac modifier icons (QA'd, the owner loves it).** Opt-in `styleData.keybindMods == "symbols"`: rewrite the
...
DECISION: glyphs stay WHITE (`:0` line-height) — the owner tried a coloured/sized variant (`|T...:px:...:r:g:b|t`
...
stretches the base round glow (uneven short vs long axis); the soft bloom forgives it and the owner said "good
...
"Cooldown & availability" Config section. Everything below QA'd in-game (the owner: "That's much better").
...
**State Highlights — Glow width + bolder rings (all three files, QA'd; matched the owner's mock screenshot).**
...
its QA'd look. (the owner chose "bolder glow", not an opaque ring. "Finish flash is fine, leave it.")
...
sweep now DARKENS a still-usable ability (1/2 charges) — the owner accepted it; dial opacity if it bugs him; (c)
...
3. **Aspect proc-glow art** (`pill-*-glow`) for tall icons (soft bloom forgives it; the owner said "good enough").
